...
for datalist0 in range(datalist.shape[0]):
    ...
    if os.path.exists(file_path):
        ...
        label = nib.load(datalist[datalist0, 6]).get_fdata()
        # print(label.shape[2])
        ...
        # data1_path = os.path.join('\\'.join(i for i in data1_list), '10', '10.nii')
        # if os.path.exists(data1_path):
        #     pass
        # else:
        #     data1_path = os.path.join('\\'.join(i for i in data1_list), '9', '9.nii')
        # if os.path.exists(data1_path):
        #     pass
        # else:
        #     missing_patien.append(f'{prefix}{datalist[datalist0, 1]}')
        #     continue
        # print(data1_path)
        # origin_data1 = nib.load(data1_path).get_fdata()
        #
        # # 对图像进行处理
        s_data = data[:, :, sclise]
        # s_odata = origin_data[:, :, sclise]
        # s_opdata = peak_data[:, :, sclise]
        # s_odata1 = origin_data1[:, :, sclise]
        #
        # min = WL - WW / 2
        # max = WL + WW / 2
        #
        # s_odata = s_odata - min
        # s_odata[np.where(s_odata > max)] = max
        #
        # s_opdata = s_opdata - min
        # s_opdata[np.where(s_opdata > max)] = max
        #
        # s_odata1 = s_odata1 - min
        # s_odata1[np.where(s_odata1 > max)] = max
        #
        # s_odata = (s_odata / (max - min)) * 255
        # s_opdata = (s_opdata / (max - min)) * 255
        # s_odata1 = (s_odata1 / (max - min)) * 255
        ...
        m_path = os.path.join(out_save, f'{prefix}{datalist[datalist0, 1]}', mask_name + '_out.png')
        print(m_path)
        cv2.imwrite(m_path, rgba_image)
        # 只有病灶内是彩色的图像
        # m1_path = os.path.join(pic_save, f'{prefix}{datalist[datalist0, 1]}', mask_name + '_in_com.png')
        m1_path = os.path.join(out_save, f'{prefix}{datalist[datalist0, 1]}', mask_name + '_out_com.png')
        ...
        foreground = rgba_image  # 使用 IMREAD_UNCHANGED 读取透明通道
        ...
        plt.savefig(os.path.join(out_save, f'{prefix}{datalist[datalist0, 1]}_temp.png'), bbox_inches='tight', pad_inches=0.1)
        plt.close()

        # 读取保存的颜色条图像
        colorbar = cv2.imread(os.path.join(out_save, f'{prefix}{datalist[datalist0, 1]}_temp.png'))
