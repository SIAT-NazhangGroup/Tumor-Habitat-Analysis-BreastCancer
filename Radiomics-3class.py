...
labels_file=r'<PROJECT_ROOT>\datas_clear.csv'
datas0_folder=r'<PROJECT_ROOT>\figure-res-0\washOut'
datas1_folder=r'<PROJECT_ROOT>\figure-res-1\washOut'
habitat_folder=r'<PROJECT_ROOT>\habitat_test'
...
for i in range(pd_all.shape[0]):
    ...
    for label in [1, 2, 3]:
        ...
        # 保存临时 mask 和 image
        tempMask = rf'<NEW_ROOT>\temp_mask_label{label}.nii'
        tempImage = rf'<NEW_ROOT>\temp_image.nii'
    ...
    # tempMask=r'<PROJECT_ROOT>\temp'+'mask.nii'
    # nib.save(mask,tempMask)

    # tempImage=r'<PROJECT_ROOT>\temp'+'image.nii'
...
# pd_result.to_csv(r'<NEW_ROOT>\radiology\washin_radiology_{}.csv'.format(today),index=False)
output_folder = r'<PROJECT_ROOT>\radiology'
