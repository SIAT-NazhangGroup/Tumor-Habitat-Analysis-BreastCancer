...
labels_file=r'E:\liuzhou_breastcancer\datas_clear.csv'
datas0_folder=r'E:\liuzhou_breastcancer\figure-res-0\washOut'
datas1_folder=r'E:\liuzhou_breastcancer\figure-res-1\washOut'
habitat_folder=r'E:\liuzhou_breastcancer\habitat_test'
...
for datalist0 in range(datalist.shape[0]):
    ...
    mask_path = os.path.join(habitat_folder, f"{grade_name}{nii_data_name}_{mask_name}.nii")
    print(nii_data_path)
    if os.path.exists(nii_data_path):
        nii_datas.append([nii_data_name, grade, label_name, nii_data_path, mask_path])
...
pd_nii_datas.columns=['patient_name','grade', 'label_name','data_path','habitat_mask']
pd_nii_datas.head()
...
list_result= {1: [], 2: [], 3: []}  # Separate results for each label
...
for i in range(pd_all.shape[0]):
    ...
    try:
        maskPath=pd_all.loc[select, 'habitat_mask']
        print(maskPath)
        mask_data = nib.load(maskPath).get_fdata()
    except:
    ...
    for label in [1, 2, 3]:
        ...
        if binary_mask.sum() == 0:
            print(f"{pd_all.loc[i, 'patient_name']} 中不包含标签 {label}")
            continue

        # 保存临时 mask 和 image
        tempMask = rf'E:\breastcancer_new\temp_mask_label{label}.nii'
        tempImage = rf'E:\breastcancer_new\temp_image.nii'
        image_data=np.array(image.get_fdata())
        nib.save(nib.Nifti1Image(binary_mask, image.affine), tempMask)
        nib.save(nib.Nifti1Image(image_data, image.affine), tempImage)

        try:
            result = extractor.execute(tempImage, tempMask)
        except:
            print(f"{pd_all.loc[i, 'patient_name']} 标签 {label} 特征提取出错")
            continue

        record = dic_result.copy()
        record['habitat'] = label
        record.update(result)
        list_result[label].append(record)
        print('habitat:', label, ' --- grade:',pd_all.loc[i,'grade'], ' ---  patient:', pd_all.loc[i,'patient_name'], ' is ok')

    # # tempImage='../data/temp/'+'image.nii'
    # tempMask=r'E:\liuzhou_breastcancer\temp'+'mask.nii'
    # nib.save(mask,tempMask)

    # tempImage=r'E:\liuzhou_breastcancer\temp'+'image.nii'
...
# pd_result.to_csv(r'E:\breastcancer_new\radiology\washin_radiology_{}.csv'.format(today),index=False)
output_folder = r'E:\liuzhou_breastcancer\radiology'

for label in [1, 2, 3]:
    records = list_result[label]
    ...
    for rec in records:
        for key in all_keys:
            if key not in rec:
                rec[key] = np.nan  # pandas 会自动识别 NaN

    # 3. 生成 DataFrame 并保存
    df_result = pd.DataFrame(records, columns=sorted(all_keys))


    # df_result = pd.DataFrame(list_result[label])
    out_path = os.path.join(output_folder, f"out_radiology_habitat{label}_{today}.csv")
    df_result.to_csv(out_path, index=False)
    print(f"📄 标签 {label} 的特征已保存到：{out_path}")
