...
for datalist0 in range(datalist.shape[0]):
    ...
    if os.path.exists(file_path):
        ...
        #     pass
        # else:
        #     data1_path = os.path.join('\\'.join(i for i in data1_list), '6', '6.nii')
        # if os.path.exists(data1_path):
        #     pass
        # else:
        #     missing_patien.append(f'{prefix}{datalist[datalist0, 1]}')
        ...
        save_sdata = s_data - min
        save_sdata[np.where(save_sdata>max)] = max
        save_sdata = (save_sdata / (max - min)) * 255
        cv2.imwrite(os.path.join(pic_save, f'{prefix}{datalist[datalist0, 1]}', f'{type}.png'), save_sdata,
                    [int(cv2.IMWRITE_JPEG_QUALITY), 100])
        # %%
